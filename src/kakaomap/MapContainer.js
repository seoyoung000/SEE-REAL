...
function MapContainer({ title, height = "70vh" }) {
  ...
  const totalDeals = complexMarkers.reduce(
    (sum, complex) => sum + (Array.isArray(complex.deals) ? complex.deals.length : 0),
    0
  );
        ...
        <header className="map-header">
          ...
          <div className="map-header__stats">
            <article>
              ...
              <strong className="stat-value">{totalComplexes.toLocaleString()}곳</strong>
            </article>
            <article>
              <p className="stat-label">최근 실거래</p>
              <strong className="stat-value">{formatPrice(trendStats.at(-1)?.avg_price)}</strong>
            </article>
            <article>
              <p className="stat-label">누적 거래</p>
              <strong className="stat-value">{totalDeals.toLocaleString()}건</strong>
            </article>
          </div>
        </header>
        ...
        <div className={`map-stage${isOpen ? " has-panel" : ""}`}>
          {isOpen && (
            <div className="map-info-overlay">
              <InfoPanel type={panelType} data={panelData} onClose={() => setIsOpen(false)} />
            </div>
          )}
        ...
        <div className="map-trend-card">
          ...
          <div className="map-trend-list">
            {trendStats.map((stat) => {
              ...
              return (
                <div key={`${stat.year}-${stat.month}`} className="map-trend-row">
